...
func (c *TargetConfigReconciler) manageScheduler(ctx context.Context, ownerReference metav1.OwnerReference) (*appsv1.Deployment, bool, error) {
	...
	// The SecondaryScheduler resource resides in a different namespace than
	// the DASOperator that manages it. Setting a namespaced owner reference
	// would make the resource invalid because cross-namespace owner
	// references are not allowed.  Simply apply the CR without any owner
	// reference so that the Secondary Scheduler Operator can reconcile it.
