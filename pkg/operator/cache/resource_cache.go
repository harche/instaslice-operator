...
// ResourceCache tracks resource usage per node
...
// ResourceEventHandlerForNode returns event handlers for Node events
func (c *ResourceCache) ResourceEventHandlerForNode() utilcache.ResourceEventHandlerFuncs {
	return utilcache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			c.nodes[n.Name] = &LocalNodeInfo{Allocatable: alloc}
			klog.V(1).Infof("Node added: %s Allocatable: CPU=%dm Mem=%d Storage=%d EphemeralStorage=%d",
...
// ResourceEventHandlerForPod returns event handlers for Pod events
func (c *ResourceCache) ResourceEventHandlerForPod() utilcache.ResourceEventHandlerFuncs {
	return utilcache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(oldObj, newObj interface{}) {
			...
			if oldP.Spec.NodeName != "" && ((oldP.DeletionTimestamp == nil && newP.DeletionTimestamp != nil) || newP.Status.Phase == v1.PodSucceeded || newP.Status.Phase == v1.PodFailed) {
				c.removePodFromNode(oldP)
				return
			}
...
func eligible(p *v1.Pod) bool {
	return p.Spec.NodeName != "" && p.DeletionTimestamp == nil && p.Status.Phase != v1.PodSucceeded && p.Status.Phase != v1.PodFailed
}

func equalRequests(a, b *v1.Pod) bool {
	ar, _ := kuResource.PodRequestsAndLimits(a)
	br, _ := kuResource.PodRequestsAndLimits(b)
	return ar.Cpu().Cmp(*br.Cpu()) == 0 && ar.Memory().Cmp(*br.Memory()) == 0 && ar.Storage().Cmp(*br.Storage()) == 0 && ar.StorageEphemeral().Cmp(*br.StorageEphemeral()) == 0
}
...
// Rebuild resets the cache from listers
func (c *ResourceCache) Rebuild(nodeLister corelisters.NodeLister, podLister corelisters.PodLister) error {
	...
	for _, n := range nodes {
		alloc := convertToLocalResource(n.Status.Allocatable)
		tmp[n.Name] = &LocalNodeInfo{Allocatable: alloc}
	}
...
// Fits checks if the pod can fit on the given node
...
func convertToLocalResource(res v1.ResourceList) LocalResource {
	...
	ephemeralQty := res[v1.ResourceEphemeralStorage]
	return LocalResource{
		MilliCPU:         cpuQty.MilliValue(),
		Memory:           memQty.Value(),
		Storage:          storageQty.Value(),
		EphemeralStorage: ephemeralQty.Value(),
	}
}
