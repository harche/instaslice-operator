	...
	if cpuRequest.Cmp(nodeAvailableCpu) < 0 && memoryRequest.Cmp(nodeAvailableMemory) < 0 {
		...
		for _, gpuuuid := range gpuUUIDs {
			...
			allocRequest, allocResult := policy.SetAllocationDetails(
				profileName,
				newStart, // TODO - replace this with Placement struct
				size,
				string(pod.UID), // TODO - replace this with podRef
				updatedInstaSliceObject.Name,
				string(inferencev1alpha1.AllocationStatusCreating), // TODO - should work with AllocationStatus instead of string conversion
				discoveredGiprofile,
				Ciprofileid,
				Ciengprofileid,
				pod.Namespace, // TODO - replace this with podRef
...
func (*InstasliceReconciler) getStartIndexFromPreparedState(instaslice *inferencev1alpha1.Instaslice, gpuUUID string, profileName string) int32 {
	...
	for _, allocResult := range instaslice.Status.PodAllocationResults {
		if allocResult.GPUUUID == gpuUUID && allocResult.AllocationStatus != inferencev1alpha1.AllocationStatusDeleted {
