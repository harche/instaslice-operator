...
func (r *InstaSliceDaemonsetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for podUID, allocResult := range instaslice.Status.PodAllocationResults {
		...
		// 1) Handle "deleting"
		if allocResult.AllocationStatus == inferencev1alpha1.AllocationStatusDeleting &&
			...
			newAlloc.AllocationStatus = inferencev1alpha1.AllocationStatusDeleted
		...
		// 2) Handle "creating"
		if allocResult.AllocationStatus == inferencev1alpha1.AllocationStatusCreating &&
			allocResult.Nodename == types.NodeName(r.NodeName) {
