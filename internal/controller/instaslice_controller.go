	...
	if !pod.DeletionTimestamp.IsZero() && isPodGated {
		...
		for _, instaslice := range instasliceList.Items {
			for podUuid, allocation := range instaslice.Status.PodAllocationResults {
				if podUuid == pod.UID && (allocation.AllocationStatus == inferencev1alpha1.AllocationStatusCreated) {
				...
				if podUuid == pod.UID && allocation.AllocationStatus == inferencev1alpha1.AllocationStatusDeleted {
					err := r.removeInstasliceAllocation(ctx, instaslice.Name, allocation)
					if err != nil {
						return ctrl.Result{}, err
					}
	...
	if isPodGated {
		...
		for _, instaslice := range instasliceList.Items {
			for uuid, allocations := range instaslice.Status.PodAllocationResults {
				if allocations.AllocationStatus == inferencev1alpha1.AllocationStatusCreated && uuid == pod.UID {
				...
				// ungating the pod.
				if allocations.AllocationStatus == inferencev1alpha1.AllocationStatusUngated && uuid == pod.UID {
					result, err := r.addNodeSelectorAndUngatePod(ctx, pod, allocations)
					if err != nil {
						return result, err
					}
...
func (r *InstasliceReconciler) podMapFunc(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	if ok {
		for uuidAllocResult, allocationResult := range instaslice.Status.PodAllocationResults {
			if allocationResult.AllocationStatus == inferencev1alpha1.AllocationStatusCreated || allocationResult.AllocationStatus == inferencev1alpha1.AllocationStatusDeleted {
...
func (r *FirstFitPolicy) SetAllocationDetails(profileName string, newStart, size int32, podUUID, nodename string,
	...
	namespace string, podName string, gpuUuid string, resourceIdentifier string, availableResourceList v1.ResourceList) (*inferencev1alpha1.AllocationRequest, *inferencev1alpha1.AllocationResult) {
	return &inferencev1alpha1.AllocationRequest{
			...
			ConfigMapResourceIdentifier: types.UID(resourceIdentifier),
			Conditions:                  []metav1.Condition{},
		}
}

// Policy based allocation - LeftToRIght
func (l *LeftToRightPolicy) SetAllocationDetails(profileName string, newStart, size int32, podUUID, nodename string,
	processed string, discoveredGiprofile int32, Ciprofileid int32, Ciengprofileid int32,
	namespace string, podName string, gpuUuid string, resourceIdentifier string, availableResourceList v1.ResourceList) *inferencev1alpha1.AllocationRequest {
	// Implement the left-to-right policy here
	return &inferencev1alpha1.AllocationRequest{}
}

// Policy based allocation - RigghToLeft
func (l *RightToLeftPolicy) SetAllocationDetails(profileName string, newStart, size int32, podUUID, nodename string,
	processed string, discoveredGiprofile int32, Ciprofileid int32, Ciengprofileid int32,
	namespace string, podName string, gpuUuid string, resourceIdentifier string, availableResourceList v1.ResourceList) *inferencev1alpha1.AllocationRequest {
	// Implement the left-to-right policy here
	return &inferencev1alpha1.AllocationRequest{}
...
func (r *InstasliceReconciler) setInstasliceAllocationToDeleting(ctx context.Context, instasliceName string, podUUID string, allocResult *inferencev1alpha1.AllocationResult, allocRequest *inferencev1alpha1.AllocationRequest) (ctrl.Result, error) {
	log := logr.FromContext(ctx)
	allocResult.AllocationStatus = inferencev1alpha1.AllocationStatusDeleting
	if err := utils.UpdateOrDeleteInstasliceAllocations(ctx, r.Client, instasliceName, allocResult, allocRequest); err != nil {
		log.Info("unable to set instaslice to state ", "state", allocResult.AllocationStatus, "pod", allocRequest.PodRef.Name)
		return ctrl.Result{Requeue: true}, err
	}
